...
def analyze_file(filename, print_data=False):
    ...
    c_coordinates = []
    for m in re.finditer('C ', molecule):
        # print(molecule[m.end(): m.end() + 39].split())
        coor_x = molecule[m.end(): m.end() + 39].split()[0]
        coor_y = molecule[m.end(): m.end() + 39].split()[1]
        coor_z = molecule[m.end(): m.end() + 39].split()[2]
        ...
        c_coordinates.append([coor_x, coor_y, coor_z])

    c_coordinates = c_coordinates[2]

    if print_data:
        print ('C atom coordinate')
        print (c_coordinates)
    ...
    # AdiabatH
    # print state_order
    if len(state_order) < len(loc_diab):
    ...
    for m in re.finditer('showmatrix diabatH', output):
        ...
        #print ('test_indices:', indices, state_order)
...
#            diabatic_energies.update({'V_h': adiabat_h * 27.2114})
    ...
    return {'c_coordinates': c_coordinates,
            'loc_diab': loc_diab,
...
for dir in dir_list:
    try:
        data = analyze_file('data/eclipsed/{}/diabatic.out'.format(dir), print_data=True)
        print (data)
        if data is not None:
